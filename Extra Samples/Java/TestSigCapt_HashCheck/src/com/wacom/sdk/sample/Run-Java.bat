...
@REM Avoid using (x86) in if..else ( command (x86) ) as this truncates the name...
@set PROGRAM_FILES=%ProgramFiles(x86)%
@if "%ProgramFiles(x86)%32BIT"=="32BIT" (
  @set PROGRAM_FILES=%ProgramFiles%
)
@set JAR_FILES="%PROGRAM_FILES%\Common Files\Florentis\flsx.jar;%PROGRAM_FILES%\Common Files\Wacom\wgssLicenceJNI.jar;"
@set DLL_PATHS="%PROGRAM_FILES%\Common Files\Florentis;%PROGRAM_FILES%\Common Files\Wacom;"
@
@echo Compiling %APP% ...
@"\Program Files\Java\jdk1.8.0_05\bin\javac" -classpath %JAR_FILES% -d bin %APP_SRC%
@echo.
@if not exist %APP_BIN% goto END
@echo Run %APP_SRC% ...
@REM skip %1% arg
@"\Program Files\Java\jdk1.8.0_05\bin\java" -classpath %JAR_FILES%.\bin -Djava.library.path=%DLL_PATHS% %APP% %2 %3 %4 %5 %6 %7
@goto END
